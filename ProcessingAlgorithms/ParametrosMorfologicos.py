...
class hCannopy(object): # Crea clase
    ...
    def __init__(self):
        ...
        self.data.plot.scatter(x='X', y='Y', ax=self.ax, s=0.5)
        self.ax.set_xlim([self.data['X'].min()-10, self.data['X'].max()+10])
        self.ax.set_ylim([self.data['Y'].min()-10, self.data['Y'].max()+10])
        cursor = Cursor(self.ax,
                        horizOn=True,
                        vertOn=True,
                        color='green',
                        linewidth=1.0)
    ...
    def onKey(self,event):
        if (event.key==' '):
            ...
            df = pd.DataFrame({'high': self.h,
                               'D_tree': self.dt,
                               'D_groove': self.dg,
                               'Diameter': self.diameter})
            df.to_csv(r'ProcessedData/Processed_data.csv',index=False,sep="\t")
        elif (event.key=='0'):
            pass
        elif (event.key=='1'):
    ...
    def h_meassure(self,df):
        ...
        for i in range(0,N):
            ...
            for j in croop.index:
                ...
                z_max = tree[k+2].max()
                h = round(z_max - self.z_ref,4)
                print('=============================================================\n'+
                      '|  Tree No. '+str(j)+', groove No. '+str(i)+'\n'+
                      '-------------------------------------------------------------\n'+
                      '|  z_max = '+str(z_max)+' mts \n'+
                      '|  Canopy height = '+str(h)+' mts \n'+
